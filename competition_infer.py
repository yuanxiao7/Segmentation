...
def parse_args():
    ...
    parser.add_argument('--use_warmup', default=False, type=eval, help='warmup')
...
class Predictor:
    ...
    def run(self, imgs_path):
        ...
        for i in range(0, len(imgs_path), args.batch_size):
            ...
            self._save_sitk(results, imgs_path[i:i + args.batch_size])
        logger.info("Finish")
    ...
    def _save_sitk(self, results, imgs_path):
        # print(imgs_path)

        for i in range(results.shape[0]):
            results = np.squeeze(results).transpose([2, 1, 0])  # x,y,z
            basename = os.path.basename(imgs_path[i])
            basename, _ = os.path.splitext(basename)
            img_itk = sitk.ReadImage(imgs_path[i])
            img_array = sitk.GetArrayFromImage(img_itk)
            image_shape = img_array.shape
            img_spacing = img_itk.GetSpacing()

            new_spacing = tuple((img_itk.GetSize() / np.array(results.shape)) * img_spacing) 
            img_origin = img_itk.GetOrigin()
            img_direction = img_itk.GetDirection()

            basename = f"{basename}.nii.gz"
            # sitk_img = sitk.GetImageFromArray(results, isVector=False)
            # sitk.WriteImage(sitk_img, os.path.join(self.args.save_dir, basename))
            
            results = results.transpose([2, 1, 0])   # z, y, x
            result_itk = sitk.GetImageFromArray(results.astype(np.uint8))
            result_itk.SetSpacing(new_spacing)
            result_itk.SetOrigin(img_origin)
            result_itk.SetDirection(img_direction)

            resampler = sitk.ResampleImageFilter()
            resampler.SetReferenceImage(result_itk)  # 需要重新采样的目标图像
            resampler.SetSize(img_itk.GetSize())
            resampler.SetOutputSpacing(img_spacing)
            resampler.SetTransform(sitk.Transform(3, sitk.sitkIdentity))
            resampler.SetInterpolator(sitk.sitkNearestNeighbor)

            mask = resampler.Execute(result_itk)  # 得到重新采样后的图像
            # print('mask shape: ', sitk.GetArrayFromImage(mask).shape)
            mask.CopyInformation(img_itk)
            sitk.WriteImage(mask, os.path.join(self.args.save_dir, basename))
